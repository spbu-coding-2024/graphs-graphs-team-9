...
class MainScreenIntegrationTestDeleteFRunTarjanSaveSQLite {
    ...
    private val initialGraph = GraphFactory.createDirectedWeightedGraph().apply {
    ...
    fun setUp() {
        ...
        coEvery { mockSqliteService.saveGraphToNewFile(any(), any(), any()) } returns Result.success("fake/path/integration_test_graph_F_deleted.db")

        viewModel = MainScreenViewModel(
                graph = initialGraph.let { originalGraph ->
    ...
    @Test
    fun userDeletesF_RunsTarjan_SavesToSQLite() = runTest(testDispatcher) {
        ...
        assertFalse("Edge F->G should not exist in the saved graph", savedGraph.getEdges().any { it.source.name == "F" || it.destination.name == "F"})
    }
}
